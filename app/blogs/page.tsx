...
export default function BlogsPage() {
  ...
  // Theme styling mirroring Home's Emerald Aurora
  ...
  // Fetch blogs from API route on load
  useEffect(() => {
    async function loadBlogs() {
      try {
        ...
        if (response.ok) {
          ...
          setBlogs(data.blogs || []);
          setLiveConnected(!!data.liveConnected);
        }
      } catch (error) {
        console.error('Failed to load Notion blogs:', error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const typesList = ['All', ...Array.from(new Set(blogs.map(b => b.type)))];
  const difficultiesList = ['All', ...Array.from(new Set(blogs.map(b => b.difficulty)))];
  const platformsList = ['All', ...Array.from(new Set(blogs.map(b => b.platform)))];
  ...
  return (
    <div className="min-h-screen bg-[#0C0C0C] text-zinc-100 flex flex-col font-sans selection:bg-emerald-500/20 selection:text-emerald-300">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-8 py-10 relative z-10 flex flex-col gap-10">
        ...
        <section className="relative">
          <div className="flex flex-col items-start gap-3">
            ...
            <h1 className="text-3xl sm:text-5xl font-light tracking-wide font-display text-white uppercase">
              OPERATIONAL <span className="text-emerald-400 text-shadow-glow">WRITEUPS</span>
            </h1>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-stretch">
            ...
            <div className="flex-grow relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-zinc-900/40 border border-zinc-805 rounded text-zinc-100 placeholder-zinc-500 text-sm focus:outline-none focus:border-emerald-500/50 transition-all font-mono"
              />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs font-mono text-zinc-500 hover:text-emerald-400"
                >
                  CLEAR
                </button>
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-zinc-900/60 pt-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="w-full bg-zinc-900/40 border border-zinc-805 px-3 py-2 rounded text-xs focus:outline-none focus:border-emerald-500/40 text-zinc-300 font-mono"
              >
                {typesList.map(type => (
        ...
        {isLoading ? (
          <div className="flex flex-col justify-center items-center py-24 gap-4">
            ...
            <p className="font-mono text-xs text-zinc-500 tracking-widest">QUERYING SEC_DATABASES UNDER NOTION_API...</p>
          </div>
        ) : (
          <div className="flex flex-col gap-16">
            
            {/* WRITEUPS RESULTS GRID OR AUTOMATED SECTIONS BY TYPE */}
            {filteredBlogs.length === 0 ? (
              ...
              Object.keys(groupedByPlatform).sort().map((platformName) => (
                <section key={platformName} className="flex flex-col gap-6 border-l-2 border-emerald-500/10 pl-4 sm:pl-6 relative">
                  ...
                  {/* Grid of cards in this platform section (strictly smaller, space-efficient 3-column) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-2">
                    {groupedByPlatform[platformName].map((blog) => (
                      <article 
                        key={blog.id}
                        className="group border border-zinc-900 bg-zinc-950/20 rounded-lg overflow-hidden flex flex-col justify-between hover:border-zinc-850 transition-all flex-grow shadow-md relative"
                      >
                        ...
                        <div>
                          ...
                          <div className="relative w-full h-32 overflow-hidden border-b border-zinc-900 bg-zinc-950 flex items-center justify-center">
                            ...
                            <img
                              src={blog.cover}
                              alt={blog.title}
                              className="relative z-10 max-w-full max-h-full object-contain p-2 opacity-80 group-hover:opacity-100 transition-all duration-300"
                              referrerPolicy="no-referrer"
                            />
      ...
      {/* FOOTER */}
      <footer className="border-t border-zinc-950 bg-black/50 py-10 mt-20 px-4 sm:px-8 text-center text-zinc-600 font-mono text-xs">
